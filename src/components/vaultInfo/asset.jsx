...
import { getAssetData } from "./vaultUtils";
...
class Asset extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      this.props.expanded === this.props.asset.id
    ) {
  ...
  componentWillUnmount() {
    ...
    emitter.removeListener(
      ERROR_WALLET_APPROVAL,
      this.walletApprovalErrorReturned
    );
    emitter.removeListener(
      ERROR_DEPOSIT_WALLET,
      this.walletDepositErrorReturned
    );
    ...
    if (apyResponseData.success) {
      this.setState({
        vaultAssetHistoricalData: {
          chartData: apyResponseData.data.chartData,
          pnl: apyResponseData.data.performanceHistory,
        },
      });
    }
  };
  ...
  setTokenIndex = () => {
    ...
    const tokenIndex = asset.depositCurrencies
      ? asset.depositCurrencies[0].tokenIndex
      : 0;
    const selectedCurrency = asset.depositCurrencies
      ? asset.depositCurrencies[0].label
      : "USDT";

    const withdrawTokenIndex = asset.withdrawCurrencies
      ? asset.withdrawCurrencies[0].tokenIndex
      : 0;
    const selectedWithdrawCurrency = asset.withdrawCurrencies
      ? asset.withdrawCurrencies[0].label
      : "USDC";
  ...
  renderErrorMessage = (classes) => {
    const { errorMessage, happyHourMessage, happyHourWarning } = this.state;
    if (errorMessage !== "") {
      return (
      ...
      <React.Fragment>
        <div className={classes.depositIputBox}>
          <TextField
            ...
            disabled={
              loading ||
              !(isDeposit ? asset.isDepositEnabled : asset.isWithdrawEnabled)
            }
            placeholder="0.00"
            variant="outlined"
          />
          <div className={classes.depositScaleContainer}>
            <div className={classes.accountInfoBlock}>
              <CurrencySelect
                currencies={
                  isDeposit ? asset.depositCurrencies : asset.withdrawCurrencies
                }
                refresh={this.state.refreshToken}
                selectedCurrency={
                  isDeposit
                    ? this.handleSelectedDepositCurrency
                    : this.handleSelectedWithdrawCurrency
                }
              />
            </div>
  ...
  calculateFeeAmount = async () => {
    ...
    let feeData = await store.getFeeInfo(
      this.props.asset,
      this.state.amount * 10 ** this.props.asset.feeDecimals
    );
  ...
  depositModalTemplate = (classes, asset) => {
    return (
      <div>
          ...
          <Grid item sm={12} xs={12}>
            <Grid item sm={12} xs={12}>
              <Table className={classes.table} aria-label="simple table">
                <TableHead>
                  <TableRow>
                    <StyledTableCellDepositHead align="left">
                      {" "}
                      <img
                        alt=""
                        src={require("../../assets/" +
                    ...
                    <StyledTableCellDepositHead align="right">
                      {this.state.amount}
                    </StyledTableCellDepositHead>
                  </TableRow>
                </TableHead>
                {!this.state.calculatingFees && (
                  <TableBody>
                    <StyledTableRow key={"emptyRow"}>
                      <StyledTableCellDeposit align="left">
                        &nbsp;
                    ...
                    <StyledTableRow key={"approveDepositValue"}>
                      ...
                      <StyledTableCellDeposit align="right">
                        {" "}
                        {this.state.finalAmount} {this.state.selectedCurrency}
                      </StyledTableCellDeposit>
                    </StyledTableRow>
                    <StyledTableRow key={"approveDepositFee"}>
                      <StyledTableCellDeposit align="left">
                        Fee({this.state.feePercentage}%)
                      </StyledTableCellDeposit>
                      <StyledTableCellDeposit align="right">
                        -{this.state.feeAmount} {this.state.selectedCurrency}
                      </StyledTableCellDeposit>
                    </StyledTableRow>
        ...
        {this.state.needVaultApproval && (
          <div className={classes.depositWarningDiv}>
            <Grid container>
              {!this.state.isApprovalErrored && (
                <Grid item sm={8} xs={8}>
                  Allow your {this.state.selectedCurrency} to be deposited in{" "}
                  {asset.strategy}
                </Grid>
              )}
              {this.state.isApprovalErrored && (
                <Grid item sm={8} xs={8}>
                  <span className={classes.erroredMessage}>
              ...
              <Grid item sm={4} xs={4} className={classes.approvalBtnBlock}>
                <Button
                  ...
                  onClick={this.getDepositApproval}
                  disabled={
                    this.state.isApprovalLoading ||
                    this.state.isApprovalCompleted
                  }
                >
                  {this.state.isApprovalLoading ? (
        ...
        {this.state.highRiskDepositWarningNeeded && (
          <div className={classes.warningCheckbox}>
            <Checkbox
              ...
              style={{ color: "white" }}
            />{" "}
            I understand that my deposit may experience high slippage due to low
            liquidity
          </div>
        )}
        {this.state.isCheckingApproval && (
          <div className={classes.erroredMessage}>
          ...
          <Grid container justifyContent="center" alignItems="center">
            <Grid item sm={8} xs={12}>
              {this.state.isDepositCompleted && (
                <span>
                  Your Token has been deposited in {asset.strategy} Strategy
                  successfully.
                </span>
              )}
              {!this.state.isDepositCompleted &&
                this.state.isDepositErrored && (
                  <span className={classes.erroredMessage}>
                    Failed to deposit Token in {asset.strategy} Strategy. Please
                    try again
                  </span>
                )}
            </Grid>
                ...
                <Button
                  ...
                  disabled={
                    ...
                    (this.state.needVaultApproval &&
                      !this.state.isApprovalCompleted) ||
                    (this.state.highRiskDepositWarningNeeded &&
                      !this.state.highRiskDepositAccepted)
                  }
                >
                  {this.state.isDepositLoading ? (
                    <CircularProgress color="#FFFFFF" size="30px" />
                  ) : this.state.isDepositCompleted ? (
                    <img src={DoneMark} alt="Done" />
                  ) : (
                    <span>Deposit</span>
  ...
  withdrawModalTemplate = (classes, asset) => {
    return (
      <div>
        <div className={classes.approvalDetails}>
          <Grid item sm={12} xs={12}>
            <span className={classes.withdrawTextStandard}>
              Kindly approve the transaction in your wallet
            </span>
          </Grid>
          <Grid item sm={12} xs={12}>
        ...
        <div className={classes.depositWarningDiv}>
          <Grid container justifyContent="center" alignItems="center">
            {this.state.isWithdrawError && !this.state.isWithdrawCompleted && (
              <Grid item sm={12} xs={12}>
                <span className={classes.erroredMessage}>
                  Failed to withdraw {this.state.selectedWithdrawCurrency}
                  <br /> from {asset.strategy} Strategy. <br />
                  Please try again
                </span>
              </Grid>
            )}
            {this.state.isWithdrawCompleted && (
              <Grid item sm={8} xs={8} className={classes.withdrawMessageBlock}>
                <span className={classes.withDrawMessage}>
                  Your {this.state.selectedWithdrawCurrency} has been withdrawn{" "}
                  <br />
                  from {asset.strategy} Strategy <br />
                  successfully.
                </span>
              </Grid>
            )}
            {this.state.isWithdrawing && (
              <Grid item sm={8} xs={8} className={classes.withdrawMessageBlock}>
                <span className={classes.withDrawMessage}>
                  Withdrawing your {this.state.selectedWithdrawCurrency} <br />{" "}
                  in {asset.strategy} Strategy
                </span>
              </Grid>
            )}
            <Grid item xs={12} />
  ...
  depositTokenToContract = () => {
    const { amount, tokenIndex, amountError, errorMessage } = this.state;
    const { asset, startLoading } = this.props;
  ...
  chooseOption = (option) => {
    if (option === "deposit") {
      this.setState({ depositClicked: true });
    } else {
      this.setState({ depositClicked: false });
    }
  };
    ...
    const modalContent = (
      <div className={classes.modalInfo}>
        <Typography variant={"h5"} className={classes.pendingInfo}>
          Funds being deployed therefore cannot be withdrawn. Please check back
          in 24-48 hrs.
        </Typography>
      </div>
    );
  ...
  render() {
    ...
    const AssetInfo = getAssetData(
      asset.asset_distribution ? asset.asset_distribution : []
    );
    ...
    return (
      <div className={classes.vaultContainer} key={asset.id}>
        <Grid container className={classes.assetSummary}>
          <Grid item xs={12}>
            <Typography className={classes.assetNameInfo} variant="h4" noWrap>
              {asset.strategyInfo}
            </Typography>
            <Typography className={classes.assetNameDescription} paragraph>
          ...
          <Box sx={{ width: "50%", display: { xs: "block", md: "none" } }}>
            ...
            >
              <Button
                className={classes.buttonNameInfo}
                onClick={() => {
                  this.chooseOption("deposit");
                }}
              >
                Deposit
              </Button>
          ...
          {this.state.isMobile ? (
            this.state.depositClicked ? (
              <React.Fragment>
                <Grid item xs={6}>
                  <Box sx={{ width: "100%", display: { xs: "block" } }}>
                    <div className={classes.thickerSepperator}></div>
                  </Box>
                </Grid>
                <Grid item sm={6} xs={12} className={classes.tradeBox}>
                  <div className={classes.actionsContainer}>
                    <div className={classes.tradeContainer}>
                      <div className={classes.operationLabel}>Deposit</div>
                      ...
                      <div>
                        <BasicModal
                          title={
                            this.state.openDepositDialogBox
                              ? "Approve Deposit"
                              : "Confirm Withdraw"
                          }
                          subTitle={
                            (this.state.openDepositDialogBox
                              ? "In "
                              : "from ") + asset.strategyName
                          }
                          contentTemplate={
                            this.state.openDepositDialogBox
                              ? this.depositModalTemplate(classes, asset)
                              : this.withdrawModalTemplate(classes, asset)
                          }
                          openModal={
                            this.state.openDepositDialogBox ||
                            this.state.openWithdrawDialogBox
                          }
                          setOpenModal={
                            this.state.openDepositDialogBox
                              ? this.setOpenModal
                              : this.setOpenWithdrawModal
                          }
                        />
                      </div>
                    ...
                    <div className={classes.tradeContainer}>
                      <div className={classes.withdrawContainer}>
                        <div className={classes.tradeContainer}>
                            ...
                            {Number(asset.pendingBalance) > 0 && (
                              <div className={classes.pendingContainer}>
                                <Typography
                                  ...
                                  noWrap
                                >
                                  <span className={classes.padding4Span}>
                          ...
                          <div
                            className={`${classes.balances} ${classes.alignCenter}`}
                          >
                            <Typography
                              variant="body1"
                              className={classes.labelMessage}
                      ...
                      <div className={classes.withdrawButtonBox}>
                        ...
                        {["daoAXA", "daoAXS", "daoASA", "daoA2S"].includes(
                          asset.vaultSymbol
                        ) &&
                          store.getStore("network") === 43113 && (
                            <Button
                              className={classes.withdrawButton}
                  ...
                  <div className={classes.tradeContainer}>
                    ...
                    <div>
                      <BasicModal
                        ...
                        subTitle={
                          (this.state.openDepositDialogBox ? "In " : "from ") +
                          asset.strategyName
                        }
                        contentTemplate={
                          this.state.openDepositDialogBox
          ...
          <Grid item xs={12}>
            <div>
              ...
              <p className={classes.assetChartDistributionBody}>
                {" "}
                {asset.strategyInfo}
              </p>
            </div>
              ...
              <TableBody>
                {AssetInfo.map((row) => (
                  <StyledTableRow
                    key={row.label}
                    hover
                    className={classes.assetRowHover}
                  >
                    <StyledTableCell component="th" scope="row">
                      <span
                        className={classes.tokenColorKey}
                      ...
                      >
                        <span className={classes.strategyCellData}>
                          {row.label}
                        </span>
                      </a>
                    </StyledTableCell>
                    <StyledTableCell
                      align="left"
                      className={classes.strategyCellData}
                    >
                      {row.percent.toFixed(2)} %
                    </StyledTableCell>
                    <StyledTableCell
                      align="right"
                      style={row.textColorStyle}
                      className={classes.strategyCellData}
                    >
                      {row.changeValue}
                    </StyledTableCell>
  ...
  renderChart = (asset) => {
    ...
    let pnl =
      this.state.vaultAssetHistoricalData &&
      this.state.vaultAssetHistoricalData.pnl
        ? this.state.vaultAssetHistoricalData.pnl.toFixed(2)
        : "0.00";
      ...
      <>
        <Grid container justifyContent="space-between">
          <Grid item xs={6} className={classes.pnlDivPosition}>
            {chartTitle}
            <span className={classes.pnlVault} style={{ color: pnlTextColor }}>
              {pnl}%
            </span>
          </Grid>
              ...
              return (
                <span
                  className={
                    ...
                    (range.value === this.state.selectedTimeRange
                      ? " " + classes.activeLabel
                      : "")
                  }
        ...
        <LineChart
          data={this.state.vaultAssetHistoricalData.chartData}
          title={""}
        />
      </>
    );
  ...
  onChange = (event) => {
    ...
    let assetTokenAmount =
      (inputAmount * asset.priceInUSD[this.state.withdrawTokenIndex]) /
      pricePerFullShareInUSD;
  ...
  verifyWithdrawInput = (shareAmount, inputAmount) => {
    ...
    if (
      !InputValidation.validateDigit(shareAmount) ||
    ...
    if (
      InputValidation.validateInputMoreThanBalance(
        inputAmount,
        displayBalance
      ) ||
  ...
  verifyInput = (amount) => {
    ...
    if (
      !InputValidation.validateDigit(amount) ||
